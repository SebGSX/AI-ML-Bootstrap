...
At the time of writing, I chose two sets of the 64GB (2x32GB) Corsair Vengeance DDR5 SDRAM with a maximum frequency of 5,600Mhz. I chose that speed because that is the maximum turbo boost frequency for my CPU and because my motherboard and chipset support it.
...
#### 4.2.4	Storage
Like with RAM, when it comes to storage think big because AI and ML need lots of space. Storage is considerably slower than RAM, which means that its speed is an important concern to avoid bottlenecks. Generally, we need to optimise storage geometry so that we trade-off capacity with speed for cost-effective performance. A good place to start is to look at the GPU’s memory speed. The maximum storage read speed should comfortably exceed the GPU’s memory speed.

To optimise storage geometry, we need to leverage a redundant array of independent disks (RAID) in such a way that the read workload is shared among all disks. RAID level 0 achieves such a requirement. Generally, the maximum storage read speed should be roughly 10 times the GPU’s memory speed. If for example your GPU’s memory speed is 15Gbps, your target maximum storage read speed is 150Gbps. Generally, the smaller capacity, lower speed NVMe SSDs provide more value per gigabyte than their larger and faster counterparts. As such, it is most cost effective to get the maximum number of drives that your motherboard will support.

To satisfy our example, if your motherboard supports four M.2 non-volatile memory express (NVMe) solid state drives (SSDs), each drive will need a maximum read speed of ~4.7GB/s to meet the 150Gbps target. Once speed is addressed through horizontal scale, scale up capacity bearing in mind that all drives must be the same size.
...
##### 5.7.3.5	Hugging Face Transformers
