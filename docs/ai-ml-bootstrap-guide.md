...
#### 4.2.4	Storage
Like with RAM, when it comes to storage think big because AI and ML need lots of space. Even the fastest storage is considerably slower than RAM, which means that its speed is an important concern to avoid bottlenecks. When considering storage performance, a lot of things matter. The mechanism of storage, the interface, and the number of storage devices working together, all contribute to storage performance.

At the time of writing, the fastest mechanism of storage is Non-Volatile Memory Express (NVMe). The fastest interface for storage devices is Peripheral Component Interconnect Express (PCIe). NVMe drives connect via PCIe, which makes such drives the natural choice for high performance. GPUs also connect via PCIe, which enables NVMe drives and GPUs to improve performance by communicating directly without the need to involve the CPU.
