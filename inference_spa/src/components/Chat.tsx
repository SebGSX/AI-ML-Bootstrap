 ...
 * @returns A JXS.Element representing the chat component.
 */
export default function Chat(): JSX.Element {
  ...
  // The message that is currently being typed, which will be sent when the user presses the send button or the enter key.
  ...
  async function postUserMessage(): Promise<void> {
    if (activeMessageText === '' || activeMessageText.trim() === '') { return; }
  ...
  return (
    <Box data-testid='chat-feed-box' sx={{pb: '3.5rem', pt: '1rem'}} height='100vh' display='flex' flexDirection='column' flex={1} overflow='auto'>
      ...
      <TextField data-testid='chat-textfield'
        ...
        fullWidth={true}
        multiline
        onChange={(event) => setActiveMessageText(event.target.value)}
        onKeyDown={handleKeyDown}
        rows={4}
        value={activeMessageText}
        variant='filled'
        // The following props are required for testing and are added to the input element.
        inputProps={{ 'data-testid': 'chat-textfield-input' }}
        // The following props are required for the send button and are specific to Material UI.
        InputProps={{
          endAdornment: (
